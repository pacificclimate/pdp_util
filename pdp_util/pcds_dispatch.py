...
class PcdsDispatcher(object):
    ...
    def __call__(self, environ, start_response):
        ...
        environ.update(
            {
                "pcic.app_root": self.kwargs["app_root"],
                "pcic.ol_path": self.kwargs["ol_path"],
            }
        )
        (
            responder_class,
            ...
            responder_kwargs,
            new_env,
        ) = self._route_request(path, environ)
    ...
    def _route_request(self, path, environ):
        ...
        try:
            ...
            return (
                HTTPNotFound,
                ["First path element after pcds/ should be 'raw' or 'climo'"],
                {},
                {},
            )  # 404
        ...
        try:
            ...
            index_kwargs.update({"is_climo": is_climo, "index_class": "network"})
            IndexClass = PcdsNetworkIndex

        try:
            station = url_parts.pop(
                0
            )  # If we can do this, it's at least a station listing
            ...
            if len(url_parts) > 0:
                if len(url_parts) == 1 and not path.endswith("/"):
                    ...
                    new_path = "../" * (
                        len(url_parts) - (0 if path.endswith("/") else 1)
                    )
