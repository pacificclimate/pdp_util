...
class RasterMetadata(object):
    ...
    def getMinMax(self, environ, start_response, req):
        ...
        with self.session_scope_factory() as sesh:
            r = (
                sesh.query(DataFileVariable.range_min, DataFileVariable.range_max)
                .join(DataFile)
        ...
        if (
            r.count() == 0
        ):  # Result does not contain any row therefore id/var combo does not exist
            start_response("404 Not Found", [])
    ...
    def getMinMaxWithUnits(self, environ, start_response, req):
        ...
        with self.session_scope_factory() as sesh:
            r = (
                sesh.query(
                    DataFileVariable.range_min,
                    DataFileVariable.range_max,
                    VariableAlias.units,
                )
                .join(DataFile)
                .join(VariableAlias)
