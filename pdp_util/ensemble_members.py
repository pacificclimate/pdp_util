...
class EnsembleMemberLister(object):
    ...
    def __call__(self, environ, start_response):
        ...
        with self.session_scope_factory() as sesh:
            ...
            if not ensemble: # Result does not contain any row therefore ensemble does not exist
                ...
                return ["Requested ensemble does not exist"]
        ...
        return dumps(self.dictify(array(tuples)))
    ...
    def dictify(self, a):
        if len(a) == 1:
            ...
            keys = set(a[:,0])
            return { key: self.dictify(a[ array([ val == key for val in a[:,0] ], dtype=bool) ,1:]) for key in keys }
