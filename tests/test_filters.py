...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"input-vars": "var1"},
            [
                func.regexp_split_to_array(cng.vars, ",\\s*").overlap(
                    postgresql.array(["var1"])
                )
            ],
        ),
        (
        ...
        (
            {"input-freq": "1-hourly,irregular"},
            [cng.freq.in_(["1-hourly", "irregular"])],
        ),
        (
        ...
        (
            {"to-date": "2000/01/01", "non-existant-filter": "bar"},
            [cng.min_obs_time < datetime(2000, 1, 1)],
        ),
        # Several valid filters
        (
            {
                "input-freq": "irregular",
                "only-with-climatology": "only-with-climatology",
                "from-date": "1890/12/25",
            },
            [
