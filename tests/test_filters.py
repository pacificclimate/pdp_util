...
@pytest.mark.parametrize(('input', 'expected'), [
    ...
    ({'input-vars': ''}, [ True ]),
    ({'input-vars': 'var1'}, [ func.regexp_split_to_array(cng.vars, '\s*').overlap(['var1']) ]),
    ({'input-vars': 'var1,var2'}, [ func.regexp_split_to_array(cng.vars, '\s*').overlap(['var1', 'var2']) ]), # OK
            ...
            for b in expected[:]:
                if isinstance(a, ColumnElement):
                    ...
                    if a.compare(b):
                        ...
                        # a.compare(b) doesn't work for non-built-in funcs,
                        # specifically `regexp_split_to_array`. To catch this
                        # case, we fall back on comparing the SQL strings
                        # generated by SQLAlchemy. This is fragile.
                        print 'Warning: fragile equality by strings:', str(a)
                        result.remove(a)
                        expected.remove(b)
                else:
                    if a == b:
